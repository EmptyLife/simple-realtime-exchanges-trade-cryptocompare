...
CCC.TRADE.unpackFast = (data) => {
	const FLAG_TRADE_ID = 0x1;
	const FLAG_TIMESTAMP = 0x2;
	const FLAG_QUANTITY = 0x4;
	const FLAG_PRICE = 0x8;
	const FLAG_TOTAL = 0x10;
	...
	const mask = parseInt(array[array.length - 1], 16);

	SubscriptionId     = +array[0];
	if ( SubscriptionId !== 0 ) {return null;}

	ExchangeName       =  array[1];
	CurrencySymbolFrom =  array[2];
	CurrencySymbolTo   =  array[3];
	Flag               = +array[4];
	...
	if ( mask === 0x1F ) {
		...
		let i = 5;
		if ( mask & FLAG_TRADE_ID  ) { TradeId   = +array[i++]; }
		if ( mask & FLAG_TIMESTAMP ) { TimeStamp = +array[i++]; }
		if ( mask & FLAG_QUANTITY  ) { Quantity  = +array[i++]; }
		if ( mask & FLAG_PRICE     ) { Price     = +array[i++]; }
		if ( mask & FLAG_TOTAL     ) { Total     = +array[i++]; }
	}
